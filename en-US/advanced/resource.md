...
Therefore, SDKs should additionally provide a proxy route `/{path}/{version}/proxy/{url}` to access such resources. This route does not require the `Satori-Platform` and `Satori-User-ID` headers.
